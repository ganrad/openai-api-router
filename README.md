...
**Model Agnostic Endpoints** | Azure OpenAI Service | The gateway (router component) exposes each AI Application through a unique endpoint, which effectively hides the underlying AI Service's backend deployments (model type, version and endpoint) from client applications. As such, AI Service deployments configured with AI applications can be updated at anytime without requiring any changes to the client applications.  This design enables quick testing/onboarding of newer model versions as soon as they become available.
