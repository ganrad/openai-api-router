...
**Flexibile Endpoint Management** | All | The gateway is AI Application Aware meaning Azure AI Service endpoints can be configured separately for each AI Application. This design not only makes it easier to share AI Service deployments among various AI Applications but also enhances metrics collection and request routing for each specific AI use case.
**GPT Model Agnostic Endpoints** | Azure OpenAI Service | The AI application endpoints exposed by the gateway abstract the model type and version from client applications. This design makes the endpoints agnostic to specific AI models and their versions. As a result, model types and versions (deployments) configured for AI applications can be changed at any time without requiring any updates to client applications.
