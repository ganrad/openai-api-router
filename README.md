...
------- | ---- | -------------------- | -----
**v2.9.6** | 02/18/2026 | Yes | This minor release includes a responsive UI refresh for the frontend application (SPA) and addresses minor issues in the AI Gateway server.
**v2.9.5** | 12/05/2025 | Yes | This latest release introduces multi level semantic cache, support for Azure OpenAI v1 API and also includes multiple bug fixes.
**v2.9.0** | 11/14/2025 | Yes | This release introduces a powerful Payload Normalization Engine that enables transformation and enrichment of both the request sent to the model deployment endpoint and the response generated by the model. This ensures that data structures conform to the expected schema for each backend/endpoint, while allowing users to enrich responses with additional attributes or metadata without writing custom code. This release also includes multiple bug fixes.
...
| **Payload Normalization** | Azure AI Foundry Models | This powerful capability enables transformation and enrichment of both request and response payloads. It's designed to give users complete flexibility in shaping data structures expected by LLM's without having to write any custom code. Payload normalization can be configured for both streaming and non-streaming API calls.<br> **Key Capabilities:**<br>- **Dynamic Payload Transformation:** Reshape request and response objects to match the schema expected by different model endpoints.<br>- **Advanced Attribute Operations:** Add, delete, or replace attributes within model input and output objects for precise control.<br>- **Schema Enrichment:** Ensure consistent data formats and enrich responses with additional metadata. |
...
| **Semantic Caching** | Azure AI Foundry Models | The gateway provides a three-tier vector cache.<br>- **Level 1:** In-memory vector cache. Ultra-fast and best for caching & retrieving limited data sets.<br>- **Level 2:** Qdrant Semantic cache. Scalable, enterprise-grade & purpose built vector cache.<br>- **Level 3**: (Default) PostgreSQL pgvector cache. Highly available & enterprise-grade.<br>Levels 1 and 2 are optional and can be selectively enabled for individual AI Applications. <br>When semantic caching is enabled, the gateway stores prompts and completions in the cache, reusing them for semantically similar queries. Caching completions significantly reduces latency and enhances efficiency, delivering up to a 40% boost in AI application performance. |
...
1. **Rapid deployment of intelligent AI Chatbots and AI Applications**
...
2. **Capturing Azure AI Service API usage metrics, cost tracking and estimating capacity for AI applications/workloads**

   For each AI Application, the gateway collects Azure AI Service API usage metrics for pre-configured time intervals. In the case of Azure AI Foundry models, the collected metrics such as *Tokens per minute* (TPM) and *Requests per minute* (RPM) can be used to estimate *Provisioned Throughput Units* for each AI workload. The gateway leverages cost budget classes to accurately calculate the API call costs for each AI application. This capability provides real-time visibility into consumption and spending, enabling organizations to monitor usage, enforce budget limits, and optimize cost allocation across multiple workloads. By automating cost computation at the application level, teams can make informed model routing decisions by utilizing pre-built endpoint routers, prevent overruns, and maintain predictable operational expenses without manual tracking.

3. **Intelligently route AI Application requests to model deployment endpoints on Azure AI Foundry**

   The AI Gateway functions as an intelligent traffic router, efficiently distributing API requests across multiple model (or service) deployment endpoints. For each AI application, users can configure one of the pre-built routing strategies (algorithms) to manage traffic flow. The gateway continuously monitors endpoint availability and performance, automatically detecting busy or unavailable endpoints and redirecting requests to healthy ones. This ensures high availability, optimized performance, and seamless failover without requiring manual intervention.  
...
**Payload Normalization** | Yes | Yes <br>- Chat Completions API | No | No | No | No | No
...
- [Qdrant Vector Database](https://qdrant.tech/documentation/concepts/search/)
...
4. Update the **AI Application Gateway Configuration file**.
   ...
   - (Optional) Use *exposeA2AEndpoint* attribute to specify if the AI Application should be exposed via an A2A protocol endpoint or not (true/false).
   ...
   - The AI Gateway supports semantic caching at 3 levels. Each level is backed by a semantic cache/store.

     - Level 1: *In-memory* vector store.  Uses *Cosine Similarity* algorithm for semantic searches.
     - Level 2: *Qdrant* vector store. Supports multiple algorithms for performing semantic searches.  Refer to the table below.
     - Level 3: *PostgreSql* vector store (Default). Supports *Cosine Similarity* and *Euclidean Distance* algorithms.

     Levels 1 and 2 are optional while level 3 serves as the default.  When all 3 levels are enabled for an AI Application, level 1 store is searched first, then level 2 and finally level 3.  When a matching entry is found, it is returned to the client application.
     ...
     Attribute Name | Type | Required | Description
     -------------- | ---- | -------- | ------------
     useCache | Boolean | Yes | AI Application Gateway will cache LLM completions (output) only when this value is set to *true*.  Default is *false*.
     searchType | String | No | AI Application Gateway will search and retrieve OpenAI Service completions based on a semantic text similarity algorithm.<br>This attribute is used to specify the similarity distance function/algorithm for Level 3 (PostgreSql) vector search.  Supported values are a) Cosine (= *Cosine Similarity*).  This is the default. b) Euclid (= *Euclidean distance*).
     searchDistance | Decimal | No | Use this attribute to specify the search similarity threshold for both Level 1 (In-memory) and Level 3 (PostgreSql) vector searches.   Specify a value between 0 and 1.
     encryptionKey | Number | No | This attribute is used to specify a 32 byte encryption key.  This key will be used to encrypt prompts and completions stored in level 2 vector store (Qdrant) only.
     level1Cache | Object | No | This attribute is used to configure level 1 (In-memory) vector store. Least recently used (LRU) strategy is used to cache and evict expired entries.
     level1Cache.entrySize | Number | No | Use this attribute to specify the max. size of an cached entry.  Default is 5MB.
     level1Cache.entryCount | Number | Yes | Use this attribute to specify the max. number of entries to store in the cache.
     level1Cache.timeToLiveInMs | Number | Yes | Use this attribute to specify the lifetime of a cached entry in milliseconds.
     level2Cache | Object | No | This attribute is used to configure level 2 (Qdrant) vector store.
     level2Cache.qdrantUri | String | Yes | Use this attribute to specify the Qdrant server URI.
     level2Cache.apiKey | String | Yes | This attribute specifies the Qdrant server API Key.
     level2Cache.searchType | String | No | This attribute is used to specify the similarity distance function/algorithm.  Supported values are a) Cosine (= *Cosine Similarity*).  This is the default. b) Euclid (= *Euclidean distance*) 3) Dot and 4) Manhattan.  Refer to the Qdrant Server documentation for more details.
     level2Cache.searchDistance | Decimal | No | Use this attribute to specify the search similarity threshold for vector searches.   Specify a value between 0 and 1.
     level2Cache.timeToLiveInMs | Number | Yes | Use this attribute to specify the lifetime of a cached entry in milliseconds.
     searchContent | Object | No | This attribute is used to configure the contents of the vector cache.
     searchContent.term | String | Yes | This value specifies the attribute in the request payload which should be vectorized and used for semantic search. For OpenAI completions API, this value should be *prompt*.  For chat completions API, this value should be set to *messages*.
     searchContent.includeRoles | String | No | This attribute value should only be set for Foundry models that expose chat completions API. Value can be a comma separated list.  Permissible values are *system*, *user* and *assistant*.
     entryExpiry | String | No | This attribute is used to specify when cached entries (*completions*) in level 3 vector store (PostgreSql) should be invalidated.  Specify, any valid PostgreSQL *Interval* data type expression. Refer to the docs [here](https://www.postgresql.org/docs/current/datatype-datetime.html#DATATYPE-INTERVAL-INPUT).
   ...
   - To enable long-term user memory (Personalization feature), specify values for attributes contained within **personalizationSettings** attribute.  Refer to the table below and set appropriate values.
     ...
     searchType | Enum(String) | No | The distance algorithm used to retrieve facts from the underlying vector store. Two algorithms are currently supported. Search type can be *Cosine* (Default) and *Euclid*.
   ...
   - To transform or enrich the API request or response (payload) routed to an AI Application model endpoint (backend), specify values for attributes contained within **normalizerSettings** attribute. Refer to the table below.

     Attribute Name | Type | Required | Description
     -------------- | ---- | -------- | -----------
     normalizerSettings | Object[] | No | An array containing normalization policy definitions

     *normalizerSettings* is an array containing normalization policy definitions. Refer to the table below and configure attributes and corresponding values appropriately.

     Attribute Name | Type | Required | Description
     -------------- | ---- | -------- | -----------
     normalizerId | String | Yes | A unique id for the normalizer policy definition.
     excludeAttributes | Object[] | No | An array containing attribute names that should be excluded from the request/response payload
     replaceAttributes | Object[] | No | An array containing attribute names that should to be replaced in the request/response payload
     includeAttributes | Object[] | No | An array containing attribute names that should be included or added to the request/response payload

     *excludeAttributes* is an array containing attribute names which should be excluded (removed) from either the model API request or response.

     Attribute Name | Type | Required | Description
     -------------- | ---- | -------- | -----------
     attributeName | String | Yes | Name of the attribute which should be removed. Use the dot `.` notation for specifying the full path to nested object attribute eg., `choices[*].content_filter_results`.  Wilcard (`*`) characters are also allowed for objects that are arrays.

     *replaceAttributes* is an array containing attribute names which should be replaced or assigned a different value.

     Attribute Name | Type | Required | Description
     -------------- | ---- | -------- | -----------
     fromAttributeName | String | Yes | Full path of the attribute (source) whose name and/or value has to be replaced
     toAttributeName | String | No | Full path of the attribute (target) which replaces the attribute specified in *fromAttributeName*
     value | Any | No | The value to be assigned to the attribute. Can be a string, number, boolean, object or an array.

     *includeAttributes* is an array containing attribute names which should be included in either the model API request or response.

     Attribute Name | Type | Required | Description
     -------------- | ---- | -------- | -----------
     attributeName | String | Yes | Full path of the attribute
     attributeValue | Any | No | Value to be assigned to the attribute. Can be a string, number, boolean, object or an array.
     op | Enum(String) | No | This is the operator which determines how the value is assigned to the attribute.  Should be set to one of these values - set (default), merge, append, insert or upsert
     broadcast | Boolean | No | When set to true and *attributeName* contains [*], apply the operator (`op`) to each resolved match (use in non-streaming mode only).
     arrayMerge | Enum(String) | No | This value specifies how to merge arrays within objects during 'merge' or upsert-merge operations. Can be set to one of these values - replace (default) or concat
     index | Number | No | Index used for 'insert' operations on array type attributes. If value specified is greater than array length, attribute is inserted at end.
     match | Object | No | Upsert matcher: find element where element[match.key] === match.value.
     match.key | String | No | Name of the attribute to match
     match.value | Any | No | Value to match. Can be a string, number, boolean, object, array or null.
     mode | Enum(String) | No | Upsert behavior when a match is found: deep-merge or replace the entire element. Can be set to one of these values - merge (default) or replace
     value | Any | No | For 'upsert': the object to insert/merge/replace as the array element. Can be a string, number, boolean, object, array or null.  
   ...
   - The **endpoints** attribute is an array containing endpoint definitions.  Refer to the table below and set appropriate values for each endpoint attribute.
     ...
     Attribute Name | Type | Required | Description
     -------------- | ---- | -------- | -----------
     id | String | Yes | An ID used to uniquely identify a *Model* deployment or an *AI Agent* endpoint defined in Azure AI Foundry Service.  The *id* assigned to each endpoint must be unique.<br>**Important Tip:** It's considered a best practice to assign unique id's to all endpoints.
     ...
     normalizationPolicy.inputNormalizerId | String | No | The unique identifier of the normalization policy definition used for transforming the request object before it is sent to the model endpoint for inferencing. This policy should be defined within the *normalizationSettings* section.
     normalizationPolicy.outputNormalizerId | String | No | The unique identifier of the normalization policy definition used for transforming the response object output by the model, before it is returned to the client application. This policy should be defined within the *normalizationSettings* section.
...
6. Run the AI Application Gateway.
   ...
   ```bash
   > ai-app-gateway@2.9.6 start
   > node --trace-warnings ./src/server.js

   02-Feb-2026 21:02:38 [info] [server.js] Starting initialization of AI Application Gateway ...
   02-Feb-2026 21:02:39 [info] [server.js] Azure Application Monitor OpenTelemetry configured.
   02-Feb-2026 21:02:39 [info] [cp-pg.js] checkDbConnection(): Postgres DB connectivity OK!
   02-Feb-2026 21:02:39 [info] [server.js] Completions will be cached
   02-Feb-2026 21:02:39 [info] [server.js] Prompts will be persisted
   02-Feb-2026 21:02:39 [info] [server.js] Conversational state will be managed
   02-Feb-2026 21:02:39 [info] [validate-json-config.js] validateAiServerSchema():
   Result:
   {
   "schema_compliant": true,
   "errors": "None"
   }
   02-Feb-2026 21:02:39 [info] [server.js] Listing AI Applications:
   ...
   Endpoints:
      ...
      Priority: 1 Uri: https://aif-project-westus3-072-resource.services.ai.azure.com/models/chat/completions?api-version=2024-05-01-preview
   ...
   02-Feb-2026 21:02:39 [info] [server.js] Cache entry invalidate run schedule (Cron) - */2 * * * *
   02-Feb-2026 21:02:39 [info] [server.js] Memory (State) invalidate run schedule (Cron) - */4 * * * *
   02-Feb-2026 21:02:39 [info] [authMiddleware.js] validateAccessToken(): AI Gateway Security Configuration:
   {
      "endpoint": "/api/v1/dev/aigateway",
      "audience": "d6f77270-40a8-4a12-ab15-f3bf812529f9",
      "issuer": "https://login.microsoftonline.com/c2236a74-6f69-4313-b77b-8a5360c27e88/v2.0",
      "algorithms": [
         "RS256"
      ],
      "requiredScopes": [
         "AiGateway.read",
         "AiGateway.write",
         "AiGateway.ControlPlane.write"
      ],
      "requiredRoles": []
   }
   02-Feb-2026 21:02:39 [info] [server.js] Server(): Azure AI Application Gateway started successfully.
   -----
   Details:
   ...
   Status: Running
   Start Date: 12/5/2025, 9:02:39 PM
   -----
   ```
