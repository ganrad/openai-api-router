...
**Flexibile Endpoint Management** | All | The gateway is AI Application Aware meaning Azure AI Service deployment endpoints can be configured separately for each AI Application. This design not only makes it easier to share AI Service endpoints among various AI Applications but also enhances metrics collection and request routing for each specific AI use case.
**Model Agnostic Endpoints** | Azure OpenAI Service | The gateway (router component) exposes each AI Application through a unique endpoint, which effectively hides the specifics of the LLM model, its version, and the AI Service's backend deployment endpoints from client applications. This design decouples client applications from model deployments and enables quick testing/onboarding of newer model versions as soon as they are released. AI Service deployments (model type, version and endpoint) configured with AI applications can be updated at anytime without requiring any changes to the client applications.
