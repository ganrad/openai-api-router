...
3. Configure AI Applications.
   ...
   search_params.auth_type | Yes | For Azure OpenAI OYD (On Your Data) API calls, this parameter has to be set to one of the values.  See below.<\b>- api_key (Set this value while authenticating to AI Search resource using API Key)<\b>- system_assigned_managed_identity (Set this value while authenticating to AI Search resource using managed identity)<\b>- user_assigned_managed_identity (Set this value while authenticating to AI Search resource using user assigned managed identity).
