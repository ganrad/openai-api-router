 ...
 * ID02232026: ganrad: v3.0.1: (Enhancement) Introduced Remote Server tools integration. This feature integrates remote tool invocation into the 
 * inference processing pipeline, allowing external context to be supplied to the LLM to improve response quality.
 * ID04252026: ganrad: v3.0.1: (Enhancement) Return the remote tools execution count as response header 'x-tools-count'.
 * ID05122026: ganrad: v3.0.1: (Enhancement) Updated the OTel http span with additional attributes - request type, app id, ...
 * ID05132026: ganrad: v3.0.1: (Enhancement) Added support for exposing EP metrics in Prometheus text exposition format.
 ...
 * ID05272026: ganrad: v3.0.1: (Enhancement) Introduced rate limit delay for AI Application endpoints.
...
const { addCustomPropertiesToSpan } = require("../config/monitor/azureMonitor-v2.js"); // ID05122026.n
const { registerClient, sendConnected, cleanupClient } = require("../utilities/sse-event-broker.js"); // ID05202026.n
...
// ID05202026.n
// Method: GET
// Description: SSE endpoint which emits status/trace events to registered clients ~ UI (RAPID/AI Gateway Console)
// Endpoint: /aigateway/events/:session_id
// Path Parameters:
//  session_id: A client/session id generated by the client/UI
router.get([GatewayRouterEndpoints.EventsEndpoint + "/:session_id"], (req, res) => {
  ...
  res.setHeader('X-Accel-Buffering', 'no'); // useful behind nginx
  res.flushHeaders?.();

  registerClient(session_id, res);
  logger.log({ level: "debug", message: "[%s] apirouter():\n  Registered Session ID: %s", splat: [scriptName, session_id] });

  // Immediately confirm connection to client
  sendConnected(session_id, { sessionId: session_id });

  req.on('close', () => {
    cleanupClient(session_id);
    logger.log({ level: "debug", message: "[%s] apirouter():\n  De-registered Session ID: %s", splat: [scriptName, session_id] });
  });
});
...
// Endpoint: /aigateway/metrics[/app_id?format=[json,prometheus]]
// Path Parameters:
//   app_id: (Optional) AI Application ID
// Query Parameters: (Default: json)
//   format: (Optional) json / prometheus
router.get([GatewayRouterEndpoints.MetricsEndpoint, GatewayRouterEndpoints.MetricsEndpoint + "/:app_id"], (req, res) => { // ID04172025.n, ID07252025.n
  ...
  if (response.responseHdrs) // ID05132026.n
    res.set(response.responseHdrs);

  if (response.format === EpMetricsOutputFormat.JSON) // ID05132026.n
    res.status(response.http_code).json(response.data);
...
// ID04232026.sn
// Method: GET
// Description: Get remote tool execution trace
// Endpoint: /aigateway/tooltrace/app_id/request_id
// Path Parameters:
//   app_id: AI Application ID
//   request_id: A Request ID generated by AI App Gateway
router.get([GatewayRouterEndpoints.ToolTraceEndpoint + "/:app_id/:request_id"], async (req, res) => { // ID04232026.n
  const response = await new ResourceHandlerFactory().getDataHandler(AppResourceTypes.AiAppGatewayToolTrace).
    handleRequest(req);
...
router.post(
  [
    ...
    const appTypes = [AzAiServices.OAI, AzAiServices.AzAiModelInfApi, AzAiServices.AzAiAgent, AzAiServices.Anthropic]; // ID11052024.n; ID03242025.n; ID06042026.n
    const spanAttributes = new Map(); // ID05122026.n
    ...
    spanAttributes.set(GatewaySpanAttrs.ApplicationID, appId); // ID05122026.n
    ...
    const apiType = ( inboundApiType === AiGatewayInboundReqApiType.OpenAI ) ? 
      ((application.appType === AzAiServices.OAI) ? inboundApiType : AiGatewayInboundReqApiType.Anthropic) : inboundApiType; // ID06042026.n
    spanAttributes.set(GatewaySpanAttrs.InboundReqType, apiType); // ID05122026.n

    if ( (req.inboundApiType === AiGatewayInboundReqApiType.OpenAI) && (apiType === AiGatewayInboundReqApiType.Anthropic) ) // ID06042026.n
      req.inboundApiType = AiGatewayInboundReqApiType.Anthropic;
    ...
    if (cdb.cacheResults && (!appConnections.getAllConnections().has(cdb.embeddApp))) {
      ...
      if (embeddApp) {
        ...
        for (const element of embeddApp.endpoints) {
          ...
          epmetrics = new EndpointMetricsFactory().getMetricsObject(embeddApp.appType, element.uri, element.id, element.rpm, element.rateLimitDelaySecs, element.healthPolicy, modelInfo); // ID04302025.n, ID05122025.n, ID08252025.n, ID05272026.n
    ...
    if (!appConnections.getAllConnections().has(appId)) {
      ...
      for (const element of application.endpoints) {
        ...
        epmetrics = new EndpointMetricsFactory().getMetricsObject(application.appType, element.uri, element.id, element.rpm, element.rateLimitDelaySecs, element.healthPolicy, modelInfo); // ID04302025.n, ID05122025.n, ID08252025.n, ID05272026.n
    ...
    if (appTypes.includes(application.appType)) {
      ...
      appConfig = {
        ...
        appRemoteServerConfig: application.remoteServerSettings // ID02232026.n
      };
    ...
    addCustomPropertiesToSpan(spanAttributes); // ID05122026.n
    ...
    if (req.body.stream && // All headers have been sent; close/end the connection
      ...
      if (response.toolsCount) { // ID04252026.n
        res_hdrs += ', ' + CustomRequestHeaders.ExecToolsCount;
        res.set(CustomRequestHeaders.ExecToolsCount, response.toolsCount); // Set the remote tools count
      };
      res.set("Access-Control-Expose-Headers", res_hdrs);
      // ID06052024.en
