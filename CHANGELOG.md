...
* **Enhancements:**
  ...
  - **Payload Normalization Engine**
    ...
    Enables transformation and enrichment of requests sent to model deployment endpoints and responses generated by models.
    ...
    Works across multiple LLM endpoints without requiring custom code.

    Benefits:
    - Seamless Multi-Model Routing – Easily route traffic to any configured LLM by reshaping payloads to match endpoint requirements.
    - Faster Integration – No need for custom adapters or middleware.
    - Batch and Streaming Support – Handles both batch inferencing and real-time streaming API calls, giving developers flexibility to choose the mode that best fits their workload and performance requirements.
    - Greater Flexibility – Enforce consistent schemas and enrich responses dynamically.
    - Reduced Complexity – Simplifies deployment in heterogeneous AI environments.
